...
if __name__ == '__main__':
    ...
    X1 = df1[['ntrips_per_day_media', 'nweekdays', 'distance_week_media', 'distance_per_trip_media', 'week', 'weekend', 'distance_under_50kmh']]
    X2 = df1[['headig_mean', 'ntrips_per_day_media', 'nweekdays', 'ntrips_under_15min', 'ntrips_between_30min_1h', 'distance_week_media', 'distance_during_night', 'distance_under_50kmh', 'distance_above_120kmh']]
    ...
    for X, Y, E in zip([X1, X2, X3, X4], [Y1, Y2, Y3, Y4], [df1['Days'], df1['Days'], df2['Days'], df2['Days']]):
        X = sm.add_constant(X)
        # E = E.replace(0, 0.01)
        poisson1 = sm.GLM(Y.values.tolist(), X.values.tolist(), family=sm.families.Poisson(sm.families.links.log()), exposure=E.values.tolist(),missing='drop').fit()
        poisson2 = TweedieRegressor(power=1, alpha=0.5, link='log').fit(X.values.tolist(), Y.values.tolist(), sample_weight=E.values.tolist())
        print(poisson1.params)
